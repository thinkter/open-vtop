...
import { SuccessMessage, WelcomeMessage, SearchResults, ContentItem, } from "./views/partials.js";
const app = new Hono();
...
app.post("/api/submit", async (c) => {
    ...
    return c.html(_jsx(WelcomeMessage, { name: name }));
});
app.get("/api/search", (c) => {
    ...
    const results = query
        ...
        : [];
    return c.html(_jsx(SearchResults, { query: query, results: results }));
});
app.delete("/api/items/:id", (c) => {
    const id = c.req.param("id");
    console.log(`Deleted item ${id}`);
    return c.html(_jsx(SuccessMessage, { message: "Item deleted successfully" }));
});
let loadMoreCount = 0;
app.get("/api/load-more", (c) => {
    loadMoreCount++;
    return c.html(_jsx(ContentItem, { count: loadMoreCount }));
});
serve({
    fetch: app.fetch,
    port: 3000,
